...
class InsightDetailsSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<InsightItemModel>(
      future: InsightItemStore().getInsightItemById(insight.data['item'], insightType.id),
      builder: (context, snapshot) {
        ...
        return Container(
            ...
            children: [
              ...
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: insight.data['recommendations'].cast<String>().map((String recommendation) => 
                      Text('\u2022 $recommendation', style: TextStyle(fontSize: 14, color: Colors.grey[700]))).toList().cast<Widget>()
              ),
              SizedBox(height: 20),
              Center(
